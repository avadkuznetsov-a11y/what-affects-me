...
# Что человек может убрать по своему решению, а что с ним просто случается.
# Предлагать «уберите аврал на работе» или «отмените перелёт» - издевательство:
# эксперимент требует, чтобы фактор был в его власти. Погода, показания кольца
# и рабочий аврал сюда не входят никогда.
CONTROLLABLE = {
    "кофе", "алкоголь", "сладкое", "выпечка", "фастфуд", "острое",
    "тренировка", "прогулка", "экран перед сном", "поздний отбой",
    "поздний ужин", "переедание", "мясо", "рыба", "овощи", "молочное",
}


def can_try(factor: str) -> bool:
    """Можно ли предложить человеку убрать этот фактор на неделю."""
    return factor in CONTROLLABLE
...
class Experiment:
    ...
    def from_link(cls, link: Link, start: date, days: int = 14) -> "Experiment":
        ...
        return cls(
            ...
            # Если от привычки становится лучше, убирать её - странное
            # предложение: «тренировка добавляет сил, уберите тренировку».
            # Такую проверяем наоборот: неделю без неё, неделю с ней.
            plan=([
                f"Дни 1–{half}: живём как обычно, ничего не меняем.",
                f"Дни {half + 1}–{days}: убираем «{link.factor}» полностью.",
                f"Каждый день отмечаем «{link.metric}» - на это уходит несколько секунд.",
            ] if link.effect < 0 else [
                f"Дни 1–{half}: обходимся без «{link.factor}».",
                f"Дни {half + 1}–{days}: возвращаем «{link.factor}» каждый день.",
                f"Каждый день отмечаем «{link.metric}» - на это уходит несколько секунд.",
            ]),
        )
